...
namespace Features.Systems
{
    ...
    public partial class FallSystem : SystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            {
                Any = new ComponentType[] { typeof(IsFallingComponent), typeof(MoveToBoardPositionComponent) },
            };

            RequireForUpdate(GetEntityQuery(queryDesc));
        }
        ...
        protected override void OnUpdate()
        {
            ...
            Entities.WithAll<IsFallingComponent>().WithNone<DestroyedComponent>().WithStructuralChanges().ForEach(
                ...
                {
                    var positionUnder = new int2(boardPositionComponent.Position.x,
                        boardPositionComponent.Position.y - 1);
                    if (boardModel.IsPositionInBorders(positionUnder) && !boardModel.HasEntityAt(positionUnder))
                    {
                        ...
                        if (EntityManager.HasComponent<MoveToBoardPositionComponent>(entity) == false)
                            EntityManager.AddComponentData(entity,
                                new MoveToBoardPositionComponent { TargetPosition = positionUnder });
                        else
            ...
            Entities.WithNone<DestroyedComponent>().WithStructuralChanges().ForEach(
                ...
                {
                    var targetPosition =
                        BoardCalculator.ConvertBoardPositionToTransformPosition(boardPositionComponent.Position);

                    var direction = targetPosition - localTransform.Position;
                    if (math.distancesq(localTransform.Position, targetPosition) < 0.01f ||
                        localTransform.Position.y < targetPosition.y)
                    {
                        localTransform = new LocalTransform { Position = targetPosition };
